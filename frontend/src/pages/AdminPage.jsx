    ...
    <div className={`${!open ? "-mt-14" : "-mt-5"} p-5 w-screen h-screen z-0`}>
      ...
      >
        <div className="bg-neutral-800 border-x border-neutral-400 w-full h-full p-5 overflow-y-auto">
          {isLoading ? (
            <AiOutlineLoading3Quarters className="animate-spin mx-auto absolute top-[50%] md:left-[58%] left-[48%] text-4xl text-sky-400" />
          ) : (
            <>
              ...
              <div className="space-y-2">
                {users.map((item) => (
                  ...
                  >
                    ...
                    <NavLink to={`/account/${item?._id}`}><div className="hover:underline text-white cursor-pointer">{item.name} <span className="text-gray-400">({item.email})</span></div></NavLink>
                  </div>
                ))}
              </div>
